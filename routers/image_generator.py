...
async def post_image(request: Request) -> JSONResponse:
    try:
        ...
        payload = {
            "model": "dall-e-3",
            "prompt": f"Generate a realistic image of a best buy technology product in its packaging in front of a plain background for a product called <{name}> with a description <{description}> to be sold in an online best buy store.",
            "n": 1
        }
